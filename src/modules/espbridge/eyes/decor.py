...
def deploy(d, W, H, now, ox=0.0, oy=0.0):  # the Docker whale -- a 'D' stacked with containers, riding a live sea -- "deploying"
...
def prompt(d, W, H, now, ox=0.0, oy=0.0):  # a terminal prompt with a blinking block cursor -- "waiting"
    ...
    if (now * 1.5) % 1.0 < 0.5:                           # blink the cursor ~ once a second
        d.rectangle([bx + 6, by, bx + 8, by + 10], fill=1)  # block cursor
